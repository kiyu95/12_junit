...
class JupiterAssertionsTests {
    ...
    /*
    * junit jupiter는 junit4 로부터 온 assertion 메소드와 새롭게 자바 8 람다 표현식으로 추가된 메소드를 제공한다.
...
//        Assertions.assertEquals(expected, result, 1); // 오차의 허용범위 지정
...
//        Assertions.assertEquals(expected, result, () -> "실패할 때 보여줄 메세지");
    ...
    void testAssertNotEqualsWithInstances(){
        ...
        // then
        Assertions.assertEquals(obj1, obj2); // 주소값이 다르기 때문에 FAILED
    }
...
//        Assertions.assertEquals(result, num2 == num1);
    ...
    void testAssertType(){
        ...
        // then : 검증하는 로직
        Assertions.assertInstanceOf(Person.class, person); // InstanceOf : 타입 비교
        Assertions.assertAll(
    ...
    /*
    * assertDoseNotThrow(executable) 메소드 호출 시 아무런 예외가 발생하지 않는지 확인
    * */
    @Test
    @DisplayName("void 메소드의 경우 exception 발생이 없이 정상적으로 동작하는지 테스트")
    void testAssertDoesNotThorw(){
...
//        );
        ...
        Assertions.assertAll(
                ...
                () -> Assertions.assertDoesNotThrow(() -> validator.checkDividableNumbers(firstNum, secondNum)), // 오류 안나는 경우
